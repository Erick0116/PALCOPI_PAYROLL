...
def payroll():
    ...
    # Fetch incentives
    incentives = supabase.table("incentive").select("*").execute().data
    incentive_map = {}
    for i in incentives:
        # Sum incentives per employee
...
def load_incentives():
    ...
    # Build incentive lookup (sum per employee)
    incentive_map = {}
    for i in incentives:
        incentive_map[i["name"]] = incentive_map.get(i["name"], 0) + float(i["amount"])
...
def save_payroll():
    ...
    for emp in employees:
        ...
        supabase.table("payroll").insert({
            ...
            "incentive": payroll_data.get("incentive", 0)  # ✅ store incentive separately
        }).execute()
