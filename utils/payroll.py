...
from config import supabase   # ✅ you already have supabase client here
...
def calculate_employee_pay(emp, attendance_records, week_number, deduction=None):
    ...
    late_grace_minutes = int(settings.get("late_grace_minutes", 5))
    late_minor_minutes = int(settings.get("late_minor_minutes", 10))
    late_major_minutes = int(settings.get("late_major_minutes", 30))
    late_deduction_hours = float(settings.get("late_deduction_hours", 1))
    late_major_deduction_hours = float(settings.get("late_major_deduction_hours", 2))
            ...
            try:
                ...
                if time_in > scheduled_start:
                    ...
                    if minutes_late > late_grace_minutes:
                        late_count += 1
                        late_minutes_total += minutes_late
    ...
    incentive_amount = 0
    try:
        incentive_record = supabase.table("incentive").select("amount").eq("name", emp["name"]).execute().data
        if incentive_record and len(incentive_record) > 0:
            incentive_amount = float(incentive_record[0].get("amount", 0))
    except Exception:
        incentive_amount = 0

    salary += incentive_amount

    # ✅ Cash Advance logic (CA first, then PCA if CA deducted)
    ca_deduction = 0
    pca_deduction = 0
    try:
        advances = supabase.table("cash_advance").select("*").eq("name", emp["name"]).execute().data

        # Deduct CA first if not yet deducted
        for adv in advances:
            if adv["type"] == "CA" and adv["status"] != "deducted":
                ca_deduction += float(adv["amount"])
                supabase.table("cash_advance").update({"status": "forreview"}).eq("id", adv["id"]).execute()

        # Deduct PCA only if CA already deducted
        ca_already_deducted = any(a["type"] == "CA" and a["status"] == "deducted" for a in advances)
        if ca_already_deducted:
            for adv in advances:
                if adv["type"] == "PCA" and adv["status"] != "deducted":
                    pca_deduction += float(adv["amount"])
                    supabase.table("cash_advance").update({"status": "forreview"}).eq("id", adv["id"]).execute()
    except Exception:
        pass

    total_cash_advance = ca_deduction + pca_deduction

    # ✅ Net pay includes incentive and cash advance
    net_pay = salary - (sss + pagibig + philhealth + total_cash_advance)
    ...
    return {
        ...
        "late_minutes": round(late_minutes_total, 2),
        "late_deduction_hours": round(late_deduction_total, 2),
        "regular_pay": round(regular_pay, 2),
        "overtime_pay": round(overtime_pay, 2),
        "incentive": round(incentive_amount, 2),
        "cash_advance": round(total_cash_advance, 2),  # NEW FIELD
